class ModernAttendanceSystem:
    ...
    def register_excused_absence(self):
        ...
        # إطار الخيارات
        options_frame = tk.Frame(excuse_window, bg=self.colors["light"], padx=20, pady=20)
        options_frame.pack(fill=tk.BOTH, expand=True)

        # متغير لتخزين الخيار المحدد
        selected_excuse = tk.StringVar(value="إجازة مرضية")
        ...
        custom_frame = tk.Frame(options_frame, bg=self.colors["light"])
        ...
        # خيار الظهور في التكميل الرسمي (للخيار "أخرى" فقط)
        ...
        def toggle_custom_fields(*args):
            """إظهار/إخفاء حقل السبب المخصص وخيار التكميل الرسمي"""
            if selected_excuse.get() == "other":
        ...
        selected_excuse.trace("w", toggle_custom_fields)
        ...
        def confirm_excuse():
            """تأكيد العذر وتسجيل الغياب"""
            # الحصول على السبب النهائي
            if selected_excuse.get() == "other":
                excuse_reason = custom_reason_entry.get().strip()
                if not excuse_reason:
                    messagebox.showwarning("تنبيه", "الرجاء إدخال سبب الغياب بعذر")
                    return

                # إضافة معلومة الظهور في التكميل الرسمي للأعذار الأخرى
                if show_in_completion_var.get():
                    excuse_reason = f"{excuse_reason} [يظهر في التكميل]"
                else:
                    excuse_reason = f"{excuse_reason} [لا يظهر في التكميل]"
            else:
                excuse_reason = selected_excuse.get()

            # إغلاق نافذة العذر
            excuse_window.destroy()

            # تسجيل الغياب بعذر
            self.insert_attendance_record("غائب بعذر", excuse_reason=excuse_reason)
